...
function FoodTrackerJournal({ journalData }) {
  ...
  return (
    <div className="p-4 space-y-6 min-h-screen ">
      ...
      <div className="bg-gray-800 not-dark:bg-emerald-600 p-4 rounded-lg">
        <h2 className="font-bold text-lg mb-2 text-white">Total Nutrition Summary</h2>
        <div className="grid grid-cols-4 gap-2">
          <div className="bg-gray-700 not-dark:bg-white p-2 rounded">
            <p className="text-sm text-gray-400">Calories</p>
            <p className="font-bold text-white not-dark:text-black">{Math.round(totals.kcal)} kcal</p>
          </div>
          <div className="bg-gray-700 not-dark:bg-white p-2 rounded">
            <p className="text-sm text-gray-400">Protein</p>
            <p className="font-bold text-white not-dark:text-black">{Math.round(totals.protein)}g</p>
          </div>
          <div className="bg-gray-700 not-dark:bg-white p-2 rounded">
            <p className="text-sm text-gray-400">Carbs</p>
            <p className="font-bold text-white not-dark:text-black">{Math.round(totals.carbs)}g</p>
          </div>
          <div className="bg-gray-700 not-dark:bg-white p-2 rounded">
            <p className="text-sm text-gray-400">Fat</p>
            <p className="font-bold text-white not-dark:text-black">{Math.round(totals.fat)}g</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 not-dark:bg-emerald-600 p-4 rounded-lg">
        ...
        <div className="flex flex-wrap gap-2">
          {uniqueIngredients.map((ingredient, index) => (
            <span key={index} className="bg-gray-700 not-dark:bg-white not-dark:text-black px-3 py-1 rounded-full text-sm text-white">
              {ingredient}
            </span>
          ))}
        </div>
      ...
      <div className="bg-gray-800 not-dark:bg-emerald-600 p-4 rounded-lg">
        ...
        {[...uniqueSicknesses].map((sickness) => (
          <div key={sickness} className="mb-4 last:mb-0">
            ...
            <p className="text-sm text-gray-400 not-dark:text-white mb-2">{consolidatedRiskLevels[sickness]?.reason}</p>
            
            <div className="bg-gray-700 not-dark:bg-white p-3 rounded">
              <h4 className="text-sm font-medium mb-1 text-white not-dark:text-black">Recommendations:</h4>
              <ul className="list-disc list-inside text-sm space-y-1 text-gray-300 not-dark:text-black">
      ...
      {mergedHighPurineIngredients.length > 0 && (
        <div className="bg-gray-800 not-dark:bg-emerald-600 p-4 rounded-lg">
          ...
          <div className="bg-gray-700 not-dark:bg-white p-3 rounded not-dark:text-black">
            <ul className="space-y-2 text-white ">
              {mergedHighPurineIngredients.map((item, index) => (
                <li key={index} className="flex justify-between not-dark:text-black">
      ...
      <div className="bg-gray-800 not-dark:bg-emerald-600 p-4 rounded-lg">
        ...
        <div className="space-y-4">
          {journalData.map((entry, index) => (
            <div key={index} className="bg-gray-700 not-dark:bg-white  p-3 rounded">
              <h3 className="font-medium text-white not-dark:text-black">Meal {index + 1}</h3>
              <div className="grid grid-cols-4 gap-2 mt-2">
                <div>
                  <p className="text-xs text-gray-400">Calories</p>
                  <p className="text-sm text-white not-dark:text-black">{entry.kcal} kcal</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">Protein</p>
                  <p className="text-sm text-white not-dark:text-black">{entry.protein}g</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">Carbs</p>
                  <p className="text-sm text-white not-dark:text-black">{entry.carbs}g</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">Fat</p>
                  <p className="text-sm text-white not-dark:text-black">{entry.fat}g</p>
                </div>
              </div>
            </div>
          ))}
